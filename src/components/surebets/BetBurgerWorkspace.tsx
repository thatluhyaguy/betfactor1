...
export default function BetBurgerWorkspace({ initialType = 'live' }: WorkspaceProps) {
  ...
  // Poll feed every 25 seconds if autoUpdate is enabled
  ...
  const filteredOpportunities = useMemo(() => {
    ...
    return list;
  }, [rawOpportunities, selectedSport, sortBy]);

  // Active selected arb (defaults to first item if none clicked)
  const activeArb = selectedArb || filteredOpportunities[0] || null;

  // Calculate Stake Splits for active selected arb
  const stakeCalculations = useMemo(() => {
    ...
    // Kenya Withholding Tax 5% on winnings
    const netProfit = Math.round((grossProfit * 0.95) * 100) / 100;

    return {
      impliedSum: L.toFixed(3),
      stakes: [
  ...
  return (
    <div className="bb-workspace">
      {/* ── TOP TRIAL ALERT BANNER ── */}
      <div className="bb-top-banner">
        <span>
          🔒 Free trial mode: only Surebets up to 1% are available. Get unlimited access with a {arbType === 'live' ? 'Live' : 'Prematch'} subscription!
        </span>
        <Link href="/pricing" className="bb-banner-cta">
          View prices →
        </Link>
      </div>
      ...
      <div className="bb-main-grid">
        ...
        <aside className="bb-sidebar">
            ...
            <div className="bb-radio-group">
              <label className="bb-radio">
                <input
                  type="radio"
                  name="arbType"
                  checked={arbType === 'live'}
                  onChange={() => setArbType('live')}
                />
                <span>● Live</span>
              </label>
              <label className="bb-radio">
                <input
                  type="radio"
                  name="arbType"
                  checked={arbType === 'prematch'}
                  onChange={() => setArbType('prematch')}
                />
                <span>○ Prematch</span>
              </label>
            </div>
          ...
          {/* Zoom */}
          <div className="bb-group">
            <label className="bb-group-label">Zoom:</label>
            <select className="bb-select" defaultValue="100%">
        ...
        <section className="bb-feed-column">
          ...
          {isLoading ? (
            ...
            <div className="bb-empty">No active surebets matching selected filters right now.</div>
          ) : (
