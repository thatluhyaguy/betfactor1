...
async function scrapeOdibets(page) {
            ...
            for (const url of endpoints) {
                try {
                    ...
                    if (r.ok) {
                        ...
                        const items = Array.isArray(j) ? j : j.data ?? j.games ?? j.events ?? [];
                        all.push(...items);
                    }
                }
                catch { /* ignore */ }
    ...
    for (const item of capturedMatches) {
        ...
        if (outcomes.length >= 3) {
            h = (0, normalize_1.sanitizeOdds)(outcomes[0]?.odd ?? outcomes[0]?.price ?? 1.01);
            d = (0, normalize_1.sanitizeOdds)(outcomes[1]?.odd ?? outcomes[1]?.price ?? 1.01);
            a = (0, normalize_1.sanitizeOdds)(outcomes[2]?.odd ?? outcomes[2]?.price ?? 1.01);
        }
        else {
            h = (0, normalize_1.sanitizeOdds)(item.home_odd ?? item.HomeOdds ?? 1.01);
            d = (0, normalize_1.sanitizeOdds)(item.draw_odd ?? item.DrawOdds ?? 1.01);
            a = (0, normalize_1.sanitizeOdds)(item.away_odd ?? item.AwayOdds ?? 1.01);
        }
        if (h > 1.01 || a > 1.01) {
            results.push({
                bookmaker: BOOKMAKER,
