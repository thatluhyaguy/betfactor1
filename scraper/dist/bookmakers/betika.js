...
async function scrapeBetika(page) {
        ...
        try {
            ...
            if (url.includes('betika.com') &&
                ...
                if (ct.includes('json')) {
                    ...
                    if (body) {
                        const raw = Array.isArray(body) ? body : body.data ?? body.matches ?? body.games ?? [];
    ...
    for (const match of capturedMatches) {
        ...
        const picks = match.picks ?? match.outcomes ?? match.markets ?? [];
        let h = 1.01, d = 1.01, a = 1.01;
        if (picks.length >= 3) {
            const findPick = (key, idx) => picks.find((p) => (p.odd_key ?? p.key ?? '').toString().toLowerCase() === key) ?? picks[idx];
            h = (0, normalize_1.sanitizeOdds)(findPick('1', 0)?.odd ?? findPick('1', 0)?.value ?? 1.01);
            d = (0, normalize_1.sanitizeOdds)(findPick('x', 1)?.odd ?? findPick('x', 1)?.value ?? 1.01);
            a = (0, normalize_1.sanitizeOdds)(findPick('2', 2)?.odd ?? findPick('2', 2)?.value ?? 1.01);
        }
        else {
            h = (0, normalize_1.sanitizeOdds)(match.home_odd ?? match.odd1 ?? match.HomeOdds ?? 1.01);
            d = (0, normalize_1.sanitizeOdds)(match.draw_odd ?? match.oddX ?? match.DrawOdds ?? 1.01);
            a = (0, normalize_1.sanitizeOdds)(match.away_odd ?? match.odd2 ?? match.AwayOdds ?? 1.01);
        }
        if (h > 1.01 || a > 1.01) {
            results.push({
                bookmaker: BOOKMAKER,
