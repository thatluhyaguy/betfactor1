...
async function scrapeSportPesa(page) {
    ...
    page.on('response', async (response) => {
        try {
            ...
            if ((url.includes('sportpesa') || url.includes('sportycs')) &&
            ...
            for (const url of endpoints) {
                try {
                    ...
                    if (r.ok) {
                        ...
                        const items = Array.isArray(j) ? j : j.events ?? j.data ?? j.matches ?? j.items ?? [];
                        all.push(...items);
                    }
                }
                catch { /* ignore */ }
    ...
    for (const item of capturedMatches) {
        ...
        if (outcomes.length >= 3) {
            h = (0, normalize_1.sanitizeOdds)(outcomes[0]?.OddsValue ?? outcomes[0]?.odd ?? outcomes[0]?.price ?? 1.01);
            d = (0, normalize_1.sanitizeOdds)(outcomes[1]?.OddsValue ?? outcomes[1]?.odd ?? outcomes[1]?.price ?? 1.01);
            a = (0, normalize_1.sanitizeOdds)(outcomes[2]?.OddsValue ?? outcomes[2]?.odd ?? outcomes[2]?.price ?? 1.01);
        }
        else {
            h = (0, normalize_1.sanitizeOdds)(item.HomeOdds ?? item.home_odd ?? item.HomeOdds ?? 1.01);
            d = (0, normalize_1.sanitizeOdds)(item.DrawOdds ?? item.draw_odd ?? item.DrawOdds ?? 1.01);
            a = (0, normalize_1.sanitizeOdds)(item.AwayOdds ?? item.away_odd ?? item.AwayOdds ?? 1.01);
        }
        if (h > 1.01 || a > 1.01) {
            results.push({
                bookmaker: BOOKMAKER,
