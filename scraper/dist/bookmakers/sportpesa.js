...
exports.scrapeSportPesa = scrapeSportPesa;
...
function parseMatch(match, scrapedAt) {
    try {
        ...
        const homeTeam = match.HomeTeam ?? match.home_team ?? match.home ?? match.competitors?.[0]?.name ?? '';
        const awayTeam = match.AwayTeam ?? match.away_team ?? match.away ?? match.competitors?.[1]?.name ?? '';
        ...
        if (outcomes.length >= 3) {
            homeOdds = (0, normalize_1.sanitizeOdds)(outcomes[0]?.OddsValue ?? outcomes[0]?.odd ?? outcomes[0]?.odds ?? outcomes[0]?.price ?? 1.01);
            drawOdds = (0, normalize_1.sanitizeOdds)(outcomes[1]?.OddsValue ?? outcomes[1]?.odd ?? outcomes[1]?.odds ?? outcomes[1]?.price ?? 1.01);
            awayOdds = (0, normalize_1.sanitizeOdds)(outcomes[2]?.OddsValue ?? outcomes[2]?.odd ?? outcomes[2]?.odds ?? outcomes[2]?.price ?? 1.01);
        }
        else {
            // Flat odds object on the match itself
            homeOdds = (0, normalize_1.sanitizeOdds)(match.HomeOdds ?? match.home_odds ?? match.odd1 ?? 1.01);
            drawOdds = (0, normalize_1.sanitizeOdds)(match.DrawOdds ?? match.draw_odds ?? match.oddX ?? 1.01);
            awayOdds = (0, normalize_1.sanitizeOdds)(match.AwayOdds ?? match.away_odds ?? match.odd2 ?? 1.01);
        }
...
async function scrapeSportPesa(page) {
    const results = [];
