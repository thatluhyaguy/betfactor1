...
function parseMatch(match: any, scrapedAt: string): ScrapedMatchOdds | null {
  try {
    const homeTeam: string =
      match.home ?? match.home_team ?? match.hometeam ?? match.HomeTeam ?? '';
    const awayTeam: string =
      match.away ?? match.away_team ?? match.awayteam ?? match.AwayTeam ?? '';
    ...
    if (outcomes.length >= 3) {
      const find = (key: string, idx: number) => {
        const byKey = outcomes.find((o) =>
          (o.bet_type ?? o.odd_key ?? o.type ?? o.outcome_key ?? '')
            .toString()
            .toLowerCase() === key
        );
      ...
      homeOdds = sanitizeOdds(find('1', 0)?.odd ?? find('1', 0)?.odds ?? find('1', 0)?.price ?? 1.01);
      drawOdds = sanitizeOdds(find('x', 1)?.odd ?? find('x', 1)?.odds ?? find('x', 1)?.price ?? 1.01);
      awayOdds = sanitizeOdds(find('2', 2)?.odd ?? find('2', 2)?.odds ?? find('2', 2)?.price ?? 1.01);
    } else {
      // Flat odds on match object
      homeOdds = sanitizeOdds(
        match.home_odd ?? match.home_odds ?? match.odd1 ?? match.HomeOdds ?? 1.01
      );
      drawOdds = sanitizeOdds(
        match.draw_odd ?? match.draw_odds ?? match.oddX ?? match.DrawOdds ?? 1.01
      );
      awayOdds = sanitizeOdds(
        match.away_odd ?? match.away_odds ?? match.odd2 ?? match.AwayOdds ?? 1.01
      );
    }
